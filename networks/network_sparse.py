...
class FullyConnectedClassifierSparse(FullyConnectedClassifier):
    ...
    def __init__(self,
        ...
        with self.graph.as_default():
            with tf.variable_scope(self.scope):
                ...
                self.params = pruning_utils.calculate_number_of_sparse_parameters(
                        self.sparse_layers)
                if self.verbose:
    ...
    def _create_placeholders(self):
        ...
        self.inputs = tf.placeholder(dtype=self.dtype, #tf.float32,#modification
                ...
                with tf.variable_scope('layer_{layer}'.format(layer=i+1)):
                    ...
                    with tf.variable_scope('sparse'):
                        ...
                        values = tf.get_variable(name='values',
                                                 ...
                                                 dtype=self.dtype)#tf.float #modification
    ...
    def getParams(self):
        return pruning_utils.calculate_number_of_sparse_parameters(
                                                            self.sparse_layers)
